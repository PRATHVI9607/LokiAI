---
source_file: "loki/requirements.txt"
type: "document"
community: "RAG Engine Semantic Document Search / Brain LLM Engine"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/RAG_Engine_Semantic_Document_Search_/_Brain_LLM_Engine
---

# Requirement: chromadb>=0.5.0 (Vector store, replaces JSON/numpy flat store)

## Connections
- [[RAG Engine Semantic Document Search]] - `implements` [EXTRACTED]
- [[Rationale ChromaDB Upgrade in requirements.txt (replaces JSONnumpy flat store)]] - `rationale_for` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/RAG_Engine_Semantic_Document_Search_/_Brain_LLM_Engine