---
source_file: "loki/config.yaml"
type: "document"
community: "RAG Engine Semantic Document Search / Brain LLM Engine"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/RAG_Engine_Semantic_Document_Search_/_Brain_LLM_Engine
---

# Config: Kimi K2 Primary LLM (Moonshot API)

## Connections
- [[Config LLM Provider Priority Kimi K2 â†’ OpenRouter â†’ Ollama]] - `references` [EXTRACTED]
- [[Rationale Kimi K2 as Primary LLM (new upgrade, Moonshot API)]] - `rationale_for` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/RAG_Engine_Semantic_Document_Search_/_Brain_LLM_Engine