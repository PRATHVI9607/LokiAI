---
source_file: "LokiPRD.md"
type: "document"
community: "RAG Engine Semantic Document Search / Brain LLM Engine"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/RAG_Engine_Semantic_Document_Search_/_Brain_LLM_Engine
---

# Brain LLM Engine

## Connections
- [[Automatic Conversation Compression at 40 Messages]] - `implements` [EXTRACTED]
- [[Brain Memory Persistent Knowledge Store (brain.json)]] - `shares_data_with` [EXTRACTED]
- [[Fact Extraction Every 5 Exchanges]] - `implements` [EXTRACTED]
- [[KORTEX 5-Layer Prompt Construction]] - `implements` [EXTRACTED]
- [[LLM Fallback Chain (Ollama â†’ OpenRouter Primary â†’ Secondary)]] - `implements` [EXTRACTED]
- [[RAG Engine Semantic Document Search]] - `shares_data_with` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/RAG_Engine_Semantic_Document_Search_/_Brain_LLM_Engine